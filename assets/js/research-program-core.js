(() => {
  ...
  if (detailFoot) detailFoot.textContent = "Pillars carry the research questions. Squares are public projects. Tags and work items open repositories.";
  ...
  function conceptRepo(label) {
    ...
    if (/(spatial|distance|geographic)/.test(x)) return REPOS.spatial;
    if (/(text|nlp|extraction|attribution|classification|evidence)/.test(x)) return REPOS.unProject;
    return REPOS.computing;
  }

  function projectRepo(label) {
    const x = String(label || "").toLowerCase();
    if (x.includes("ai governance and non-proliferation")) return REPOS.aiProject;
    if (x.includes("islamic state attack patterns")) return REPOS.isProject;
    if (x.includes("un transcript and voting alignment")) return REPOS.unProject;
    return null;
  }

  function nodeRepo(id) {
    return ({
      unsupervised:REPOS.unsupervised,clustering:REPOS.clustering,dimension:REPOS.dimension,networks:REPOS.networks,
      longitudinal:REPOS.longitudinal,missing:REPOS.missing,bayesian:REPOS.computing,multilevel:REPOS.longitudinal,
      validation:REPOS.computing,nlp:REPOS.unProject,spatial:REPOS.spatial,forecasting:REPOS.computing,
      political:REPOS.political,massviolence:REPOS.political,counterextremism:REPOS.terrorism,humanecology:REPOS.ecology,
      humanitarian:REPOS.humanitarian,humansecurity:REPOS.humansecurity,climate:REPOS.ecology,
      ainonproliferation:REPOS.technology,autonomous:REPOS.technology,proj_ai:REPOS.aiProject,proj_is:REPOS.isProject,proj_un:REPOS.unProject
    })[id] || null;
  }
  ...
  map.innerHTML = `<title id="rpm-title">Research program network</title><desc id="rpm-desc">Computational statistics connected to three research pillars, methods, application domains, and public project repositories.</desc><g>${HALOS.map(haloMarkup).join("")}</g><g>${EDGES.map(edgeMarkup).join("")}</g><g>${Object.entries(LAYOUT).map(([id,node])=>nodeMarkup(id,node)).join("")}</g>`;
  ...
  function renderDetail(id) {
    ...
    kicker.textContent=d.kind==="core"?"Research identity":d.kind==="pillar"?"Research pillar":d.kind==="method"?"Method":d.kind==="domain"?"Application domain":"Project";
    title.textContent=d.title; summary.textContent=d.summary;
    ...
    const linked=d.work.map(label=>({label,href:projectRepo(label)})).filter(x=>x.href), block=projects.closest(".rpm-detail-block");
    if(linked.length){block.hidden=false;projects.innerHTML=linked.map(x=>`<li><a class="rpm-work-link" href="${x.href}" target="_blank" rel="noopener noreferrer">${x.label}</a></li>`).join("");}else{block.hidden=true;projects.innerHTML="";}
  }
  ...
  nodes.forEach(node=>{const id=node.dataset.id;node.addEventListener("mouseenter",()=>highlight(id,true));node.addEventListener("mouseleave",restore);node.addEventListener("focus",()=>highlight(id,true));node.addEventListener("blur",restore);node.addEventListener("click",()=>{const d=DATA[id];if(d.kind==="project"&&d.url){window.open(d.url,"_blank","noopener,noreferrer");return;}if(id==="center"||locked===id)overview();else highlight(id);});node.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();node.click();}});});

  function renderMobile(){if(!mobile)return;mobile.innerHTML=MOBILE_GROUPS.map((g,i)=>`<div class="rpm-mobile-item${i===0?" is-open":""}"><button class="rpm-mobile-trigger" type="button" aria-expanded="${i===0?"true":"false"}"><span class="rpm-mobile-name">${g.title}</span><span class="rpm-mobile-symbol">${i===0?"−":"+"}</span></button><div class="rpm-mobile-content"><p>${g.summary}</p>${g.ids.map(id=>{const href=DATA[id].url||nodeRepo(id);return href?`<a href="${href}" target="_blank" rel="noopener noreferrer">${DATA[id].title}</a>`:`<span>${DATA[id].title}</span>`;}).join("")}</div></div>`).join("");}
